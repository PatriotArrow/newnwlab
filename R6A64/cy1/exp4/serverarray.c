#include <stdio.h>
#include <stdlib.h>
#include <string.h>
#include <unistd.h>
#include <sys/socket.h>
#include <netinet/in.h>

#define PORT 8080
#define MAX_CONNECTIONS 5
#define BUFFER_SIZE 1024

int main(int argc, char const *argv[]) {
    int server_fd, new_socket, valread;
    struct sockaddr_in address;
    int opt = 1;
    int addrlen = sizeof(address);
    char buffer[BUFFER_SIZE] = {0};
    int array_size, array[MAX_CONNECTIONS], i;

    // Create socket file descriptor
    if ((server_fd = socket(AF_INET, SOCK_STREAM, 0)) == 0) {
        perror("socket failed");
        exit(EXIT_FAILURE);
    }

    // Forcefully attaching socket to the PORT
    if (setsockopt(server_fd, SOL_SOCKET, SO_REUSEADDR | SO_REUSEPORT, &opt, sizeof(opt))) {
        perror("setsockopt");
        exit(EXIT_FAILURE);
    }
    address.sin_family = AF_INET;
    address.sin_addr.s_addr = INADDR_ANY;
    address.sin_port = htons( PORT );

    // Bind the socket to the address and port specified above
    if (bind(server_fd, (struct sockaddr *)&address, sizeof(address))<0) {
        perror("bind failed");
        exit(EXIT_FAILURE);
    }

    // Listen for incoming connections
    if (listen(server_fd, MAX_CONNECTIONS) < 0) {
        perror("listen");
        exit(EXIT_FAILURE);
    }

    // Accept incoming connections and read data from them
    while ((new_socket = accept(server_fd, (struct sockaddr *)&address, (socklen_t*)&addrlen))) {
        // Read the size of the array from the client
        valread = read(new_socket, buffer, BUFFER_SIZE);
        array_size = atoi(buffer);

        // Read the array from the client
        for (i = 0; i < array_size; i++) {
            valread = read(new_socket, buffer, BUFFER_SIZE);
            array[i] = atoi(buffer);
        }

        // Calculate the average, minimum, and maximum of the array
        int sum = 0;
        int min = array[0];
        int max = array[0];
        for (i = 0; i < array_size; i++) {
            sum += array[i];
            if (array[i] < min) {
                min = array[i];
            }
            if (array[i] > max) {
                max = array[i];
            }
        }
        double average = (double)sum / array_size;

        // Send the results back to the client
        char result_buffer[BUFFER_SIZE];
        sprintf(result_buffer, "%.2f %d %d", average, min, max);
        send(new_socket, result_buffer, strlen(result_buffer), 0);
        close(new_socket);
    }
    return 0;
}
